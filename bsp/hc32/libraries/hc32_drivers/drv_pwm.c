...
static rt_err_t pwm_tmr4_init(struct hc32_pwm_tmr4 *device)
{
    ...
    for (i = 0; i < TMR4_CHANNEL_NUM_MAX; i++)
    {
        if ((device->channel >> i) & 0x01)
        {
            ...
            TMR4_PWM_Init(TMR4x, i/2, &device->stcTmr4PwmInit);
...
static void tmr6_duyt100or0_output(CM_TMR6_TypeDef *TMR6x, uint32_t channel, uint32_t CompareValue)
{
    if (CompareValue <= 1)
    {
        TMR6_PWM_SetForcePolarity(TMR6x, channel, TMR6_PWM_FORCE_LOW);
    } else if (CompareValue == (TMR6_GetPeriodValue(TMR6x, TMR6_PERIOD_REG_A) + 1))
    {
        TMR6_PWM_SetForcePolarity(TMR6x, channel, TMR6_PWM_FORCE_HIGH);
    } else
    {
