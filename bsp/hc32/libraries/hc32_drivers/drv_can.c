...
static uint32_t _get_filter_idx(struct rt_can_filter_config *p_filter_in)
{
    ...
    {
        if (p_filter_in->items[i].hdr != -1)
        {
            filter_selected |= 1 << p_filter_in->items[i].hdr;
        }
    }
...
static rt_bool_t _check_filter_params(struct rt_can_filter_config *p_filter_in)
{
    ...
    for (int i = 0; i < p_filter_in->count; i++)
    {
        if (p_filter_in->items[i].hdr != -1 && p_filter_in->items[i].hdr >= FILTER_COUNT)
        {
            RT_ASSERT(p_filter_in->items[i].hdr < FILTER_COUNT);
            return RT_FALSE;
        }
...
static int _ops_can_sendmsg(struct rt_can_device *can, const void *buf, rt_uint32_t box_num)
{
...
static int _ops_can_recvmsg(struct rt_can_device *can, void *buf, rt_uint32_t fifo)
{
    int32_t ll_ret;
