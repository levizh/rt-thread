...
#define AES128_UNIT_BIT     (128U)
#define AES128_UNIT_BYTE    (AES128_UNIT_BIT / 8U)
/* local varible */
...
rt_uint8_t plain_text[AES128_UNIT_BYTE];
rt_uint8_t cipher_text[AES128_UNIT_BYTE];

/* aes driver test */
static void hw_aes_crypt(const rt_uint8_t in[AES128_UNIT_BYTE],
                        rt_uint8_t out[AES128_UNIT_BYTE], hwcrypto_mode mode)
{
    rt_err_t ret;
    ...
    rt_hwcrypto_symmetric_crypt(ctx, mode, AES128_UNIT_BYTE, in, out);
    if(RT_EOK != ret)
    {
...
static void crypto_aes_sample(void)
{
    ...
    cypto_data_init(plain_text, AES128_UNIT_BYTE);
    rt_kprintf("\norigin plain text(hex):\n");
    for(i = 0; i < AES128_UNIT_BYTE; i++)
    {
    ...
    rt_memset(cipher_text, 0, AES128_UNIT_BYTE);
    /* aes encrypt */
    ...
    rt_memset(plain_text, 0, AES128_UNIT_BYTE);
...
void crypto_sample(int argc, char *argv[])
{
    if(argc < 2)
    {
        ...
        rt_kprintf("%*c %d aes-128 test sample\n", 4, ' ', CMD_AES);
